"""
Provider IA Groq — génération de contenu contextuel pour le simulateur Jira.
Utilise Llama 3.3 70B Versatile (tier gratuit : ~14000 req/jour).
"""
import os
import logging
import random
...
class GroqProvider(BaseProvider):
    """Provider Groq / Llama 3.3 70B Versatile pour la génération de contenu simulé."""
    ...
    def __init__(self) -> None:
        try:
            ...
            logger.info("GroqProvider initialisé avec le modèle %s", self.MODEL)
        except Exception as e:
            logger.error("GroqProvider : échec d'initialisation — %s", e)
            self._client = None

    def generate(self, event: Dict) -> str:
        """Génère un texte contextuel pour l'événement via Groq."""
        ...
        try:
            ...
            text = content.strip()
            logger.debug("Groq → %s caractères générés", len(text))
            return text
        except Exception as e:
            logger.warning("Groq erreur API : %s — utilisation du fallback", e)
            return self._fallback(event)
...
"""))
    ...
    def _fallback(self, event: Dict) -> str:
        """Retourne une réponse de fallback si Groq est indisponible."""
